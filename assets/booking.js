...
const OASIS_DEFAULT_THEME = {
  primaryHex: "#0b3d2e",
  bannerDesktop: "./assets/banners/oasis-desktop.png",
  bannerMobile: "./assets/banners/oasis-mobile.png",
};

function landingTheme(placeId) {
  let stored = {};
  try {
    stored = JSON.parse(memoryGet(`turnoya-theme-${placeId}`) ?? "null") ?? {};
  } catch {
    stored = {};
  }
  if (placeId !== "oasis") return stored;
  return {
    ...OASIS_DEFAULT_THEME,
    ...stored,
    bannerDesktop: stored.bannerDesktop || OASIS_DEFAULT_THEME.bannerDesktop,
    bannerMobile: stored.bannerMobile || OASIS_DEFAULT_THEME.bannerMobile,
  };
}
...
function whatsappMarkSvg() {
  return `<svg class="wa-mark-ico" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15s-.77.97-.94 1.17-.35.22-.64.07-1.26-.46-2.4-1.47c-.88-.79-1.48-1.76-1.65-2.06-.18-.3-.02-.46.13-.61.14-.13.3-.35.45-.52.14-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.61-.92-2.21-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.28.3-1.04 1.02-1.04 2.48s1.07 2.87 1.21 3.07c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.63.71.23 1.36.2 1.87.12.57-.08 1.76-.72 2.01-1.41.25-.7.25-1.29.17-1.41-.07-.13-.27-.2-.57-.35m-5.42 7.4h-.01a9.87 9.87 0 0 1-5.03-1.38l-.36-.21-3.74.98 1-3.65-.24-.37A9.86 9.86 0 0 1 2.17 11.9C2.17 6.45 6.6 2.01 12.06 2.01c2.64 0 5.12 1.03 6.99 2.9a9.83 9.83 0 0 1 2.89 6.99c0 5.45-4.44 9.88-9.89 9.88m8.41-18.3A11.82 11.82 0 0 0 12.05 0C5.5 0 .16 5.34.16 11.89c0 2.1.55 4.14 1.59 5.95L.06 24l6.3-1.65a11.88 11.88 0 0 0 5.69 1.45h.01c6.55 0 11.89-5.34 11.89-11.89 0-3.17-1.24-6.16-3.49-8.41"/></svg>`;
}
